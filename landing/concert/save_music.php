...
if(!isset($resp['status'])){
    if(empty($id)){
        $sql = "INSERT INTO `concert` set {$data}";
    }else{
        $sql = "UPDATE `concert` set {$data} where id = '{$id}'";
    }
    $save = $conn->query($sql);
    if($save){
        $mid = !empty($id) ? $id : $conn->insert_id;
        $resp['status'] = 'success';
        $resp['msg'] = ' Concert successfully saved';
        ...
        if(!empty($_FILES['img']['tmp_name'])){
            ...
            if(strpos($type,"image/") > -1){
                ...
                if($move){
                    $conn->query("UPDATE `concert` set image_path = CONCAT('./images/{$filename}?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$mid}'");
                }else{
