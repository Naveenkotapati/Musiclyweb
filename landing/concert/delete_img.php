...
$query = $conn->query("SELECT * FROM concert where id = '{$id}'");
if($query->num_rows > 0){
    $res = $query->fetch_array();
    $title=$res['title']; 
    
    $delete = $conn->query("DELETE FROM `concert` where id = '{$id}'");
    if($delete){
        ...
        echo '<script>alert("Concert successfully deleted."); location.replace("http://localhost/music/landing/concert/addsongs.php?title='. $res['title'] . ' ")</script>';
    }else{
